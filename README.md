...
本仓库用于集中存放可复用的 Codex Skills，便于在多台设备之间同步、版本管理和按任务复用。每个 skill 通常是一个独立目录，目录内包含 `SKILL.md`、可选脚本、参考资料、模板资产和 agent 配置。
...
| `feishu-oauth-login-setup` | 飞书 OAuth 登录配置与问题定位。 |
...
- 优先阅读目标目录下的 `SKILL.md`，只加载当前任务需要的参考文件。
- 修改 skill 后先做 dry-run 或局部验证，再执行 apply / sync。
- 每次同步建议保持“一次一个 skill、一次一个 commit”，便于回溯。
...
3. 运行该 skill 自带的验证脚本；若无脚本，至少人工检查 Markdown 和路径引用。
4. 更新本 README 的清单和用途说明。
5. 提交单一主题 commit。

## Notes

本仓库是 skill 内容仓库，不是运行时应用。具体执行方式取决于使用这些 skills 的 Codex / agent 环境。
